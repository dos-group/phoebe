...
class WorkloadModelImpl(BasePredictionModel):
    def __init__(self, *args, **kwargs):
        ...
        self.num_models: int = 20
        self.step_size: int = 30
    ...
    def eval_prediction_accuracy(self, workload: TimeSeries):
        if self.predicted_workload is not None:
            ...
            if sub_predicted_workload.count >= workload.count:
                _, true_thr_rates = TimeSeries.unfold(workload)
                ...
                reg_res: str = WorkloadModelImpl.get_regression_results(true_thr_rates,
                                                                        pred_thr_rates[:len(true_thr_rates)])
                logging.info(f"Predict-Performance of Workload-Regressor (last {workload.count}s): {reg_res}")
...
class WorkloadModelProvider(BaseProvider):
    def __init__(self):
        super().__init__(WorkloadModelImpl, "workload")

    @staticmethod
    @lru_cache()
    def get_instance():
        return WorkloadModelProvider()
