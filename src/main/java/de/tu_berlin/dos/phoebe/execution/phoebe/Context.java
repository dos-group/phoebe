...
public class Context implements AutoCloseable {
    ...
    public final int maxRecTimeConst;
    ...
    public final Workload.Iterator workIterator;
    public final List<Job> profiles = new ArrayList<>();
    ...
    private Context(String propertiesFile) throws Exception {
        ...
        this.maxRecTimeConst = Integer.parseInt(props.getProperty("general.maxRecTimeConst"));
        ...
        // get and assign profiling workloads and jobs
        ...
        String generatorType = props.getProperty("general.generatorType");
        int stepSize = Integer.parseInt(props.getProperty("general.proStepSize"));
        int firstRate = Integer.parseInt(props.getProperty("general.proFirstRate"));
        this.workIterator = Workload.iterator(generatorType, brokerList, tarConsTopic, stepSize, firstRate);
        ...
        int scaleOutCount = Integer.parseInt(props.getProperty("general.scaleOutCount"));
        for (int scaleOut : calcRange(maxScaleOut, minScaleOut, scaleOutCount)) {

            String jobName = String.format("profile_%d_%s", scaleOut, uniqueStr);
            Job job = new Job(jobName, brokerList, tarConsTopic, tarProdTopic, entryClass, chkInterval);
            ...
            this.profiles.add(job);
        }
        // configure target job
        this.job = new Job("phoebe", this.brokerList, this.tarConsTopic, this.tarProdTopic, entryClass, chkInterval);
        PropertyTree misc = props.getPropertyList("misc").find("args");
        if (misc != null) misc.forEach(e -> job.extraArgs.put(e.key, e.value));
        job.setScaleOut(6);
        // configure failures
